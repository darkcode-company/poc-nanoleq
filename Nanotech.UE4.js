...
console.log("Emscripten version: E:/Epic Games/UE_4.23/Engine/Extras/ThirdPartyNotUE/emsdk/emscripten/1.38.31");
console.log("Emscripten configuration: E:/Epic Games/UE_4.23/Engine/Intermediate/Build/HTML5/.emscripten");
...
Module.arguments = ['../../../Nanotech/Nanotech.uproject','-stdout',];
...
function resizeCanvas(aboutToEnterFullscreen) {
	...
	var minimumCanvasHeightFractionOfBrowserWindowHeight = 0.85; // and also vertically take up this much % of the total browser client area height.
...
//	}
	...
	function withIndexedDB(db) {
		...
		// ----------------------------------------
		// MORE JS
		var dataJsDownload = fetchOrDownloadAndStore(db, Module.locateFile('Nanotech.data.js'));
...
/* // The following code would download and store the .data file as a Blob, which should be more efficient than loading an ArrayBuffer. However that seems to be buggy, so avoid it for now.
			fetchOrDownloadAndStore(db, Module.locateFile('Nanotech.data')).then(function(dataBlob) {
				return readBlobToArrayBuffer(dataBlob).then(function(dataArrayBuffer) {
					Module['preloadedPackages'] = {};
					Module['preloadedPackages'][Module.locateFile('Nanotech.data')] = dataArrayBuffer;
...
// Instead as a fallback, download as ArrayBuffer. (TODO: Figure out the bugs with the above, and switch to using that one instead)
			fetchOrDownloadAndStore(db, Module.locateFile('Nanotech.data'), 'arraybuffer').then(function(dataArrayBuffer) {
				Module['preloadedPackages'] = {};
				Module['preloadedPackages'][Module.locateFile('Nanotech.data')] = dataArrayBuffer;
				return dataJsDownload.then(addScriptToDom);
			});
